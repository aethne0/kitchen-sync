...
/// Used by callers to attach themselves into the "queue" link-list.
...
impl<T> MCSLock<T> {
    ...
    pub fn lock<'a>(&'a self, queue_node: &'a mut QueueNode) -> Guard<'a, T> {
...
impl<'a, T> Drop for Guard<'a, T> {
    fn drop(&mut self) {
        ...
        let queue_node_ptr = self.queue_node as *mut _;

        // Has anyone attached themselves "after" us while we were working?
        while self.queue_node.next.load(Ordering::Acquire).is_null() {
        ...
        unsafe {
            let locked_field = &(*self.queue_node.next.load(Ordering::Relaxed)).locked;
